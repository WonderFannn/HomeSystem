...
public class Basic extends BaseImplementation {
	...
	public SipProfile buildAccount(SipProfile account) {
		...
		account.display_name = "buildByxap";
		
//		String[] serverParts = accountServer.getText().split(":");
		account.acc_id = "<sip:bb@sffd>";//"<sip:" + SipUri.encodeUser(accountUserName.getText().trim()) + "@"+serverParts[0].trim()+">";
		
		String regUri = "sip:" + "sffd"; //accountServer.getText();
		account.reg_uri = regUri;
		account.proxies = new String[] { regUri } ;


		account.realm = "*";
		account.username = "xap";//getText(accountUserName).trim();
		account.data ="xap";// getText(accountPassword);
